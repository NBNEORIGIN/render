...
def amazon_publish_products():
    ...
    Groups products by description so each sign design becomes its own
    Amazon parent-child variation listing (one parent per design, 15 children
    for 5 sizes × 3 colours). Upserts catalogue_listing + catalogue_variant
    rows, auto-assigns EANs from pool, then calls publish_listing() per group.
    """
    import re, json as _json
    from amazon_api import publish_listing

    data = request.json or {}
    # Optional: publish only one specific design by description
    filter_desc = (data.get('theme') or '').strip()
    ...
    # Group products by description — one Amazon listing per sign design
    from collections import defaultdict
    by_desc = defaultdict(list)
    for p in products:
        desc = (p.get('description') or 'Sign').strip()
        if not filter_desc or desc.lower() == filter_desc.lower():
            by_desc[desc].append(p)

    if not by_desc:
        return jsonify({"error": f"No products matching '{filter_desc}'"}), 400
    ...
    try:
        ...
        for theme, group_products in sorted(by_desc.items()):
            internal_ref = re.sub(r'[^A-Z0-9_]', '',
                                  theme.upper().replace(" ", "_").replace("-", "_"))

            # Upsert catalogue_listing for this design
            cur.execute("""
            ...
            """, (
                ...
                f"{theme} Sign – Brushed Aluminium, Weatherproof, Self-Adhesive.",
            ...
            # Upsert catalogue_variant per product in this group
            for p in group_products:
                ...
                color_display = COLOR_DISPLAY.get(color, color.title())
                price = SIZE_PRICING.get(size, 12.99)
                title_full = (f"{theme} Sign – {length_cm:.0f}x{width_cm:.0f}cm "
                              f"Brushed Aluminium {color_display}")
                image_urls = [
        ...
        return jsonify({"listings": all_results, "total": len(all_results)})
